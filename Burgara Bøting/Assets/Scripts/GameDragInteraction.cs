...
public class GameDragInteraction : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Mouse.current.leftButton.wasPressedThisFrame || Touchscreen.current?.primaryTouch.press.wasPressedThisFrame == true)
        {
            ...
            // ✅ ADDED: Reset connected pairs
            connectedNodePairs.Clear();

            botaNal.SetActive(true);
        }
            ...
            if (hit != null)
            {
                ...
                if (currentNotaNode != null)
                {
                    ...
                    if (touchedNotaNodesAmount > 0 && currentNotaNode != touchedNotaNodes[touchedNotaNodesAmount - 1])
                    {
                        ...
                        // ✅ ADDED: Prevent reconnecting same segment in any direction
                        if (connectedNodePairs.Contains((lastNotaNode, currentNotaNode)) ||
                            connectedNodePairs.Contains((currentNotaNode, lastNotaNode)))
                        {
                            return;
                        }
    ...
    private void TempConnectNodes(NotaNode currentNotaNode, Vector3 pointerPos)
    {
        ...
        // ✅ ADDED: Record this edge (both directions)
        NotaNode lastNode = touchedNotaNodes[touchedNotaNodesAmount - 2];
        connectedNodePairs.Add((lastNode, currentNotaNode));
        connectedNodePairs.Add((currentNotaNode, lastNode));
